...
<div class="max-w-3xl mx-auto py-8">
  <div class="p-6 bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-xl">
    ...
    @if($items->isEmpty())
      ...
      <a href="{{ route('catalog.index') }}" class="inline-block bg-gradient-to-r from-purple-500 to-pink-500 hover:from-pink-500 hover:to-purple-500 text-white font-semibold py-2 px-6 rounded-2xl shadow-lg transition transform hover:scale-105">
        Ver catálogo
      </a>
    @else
            ...
            <tr class="border-b border-white/20 hover:bg-white/20 transition rounded-lg">
              ...
              <td class="p-4">
                <form action="{{ route('cart.remove', $juego) }}" method="POST">
                  @csrf
                  @method('DELETE')
                  <button class="text-pink-400 hover:text-pink-300 font-semibold transition">
                    Eliminar
                  </button>
                </form>
              </td>
